...
package com.juyan.shortlink.gateway.filter;

import com.alibaba.fastjson2.JSON;
import com.alibaba.fastjson2.JSONObject;
import com.juyan.shortlink.gateway.config.Config;
import com.juyan.shortlink.gateway.dto.GatewayErrorResult;
...
public class TokenValidateGatewayFilterFactory extends AbstractGatewayFilterFactory<Config> {
    ...
    public GatewayFilter apply(Config config) {
        return (exchange, chain) -> {
            ...
            //查看请求是否在白名单
            if (!isPathInWhiteList(requestPath, requestMethod, config.getWhitePathList())) {//不在白名单
